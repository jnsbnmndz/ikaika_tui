...
class SettingsCapability(Capability):
    ...
    async def execute(self) -> int:
        while True:
            ...
            # file this one is about to show.
            values = await self._console.open_run_panel(
                "Settings", self._options(self._config.settings())
            )
            if values is None:
                return CANCELLED
    ...
    def _templates(self, values: OptionValues) -> dict[str, TemplateSource]:
